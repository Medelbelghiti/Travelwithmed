...
async function main() {
  console.log("Seeding Roamora…");

  // ---------- Admin user ----------
  const adminEmail = process.env.ADMIN_EMAILS?.split(",")[0]?.trim() ?? "admin@roamora.com";
  const password = process.env.SEED_ADMIN_PASSWORD ?? "roamora-admin";
  const passwordHash = await bcrypt.hash(password, 12);
  await prisma.user.upsert({
    where: { email: adminEmail },
    update: { role: "ADMIN", passwordHash, name: "Roamora Admin" },
    create: { email: adminEmail, role: "ADMIN", passwordHash, name: "Roamora Admin", isActive: true },
  });
  ...
  const products = [
    ...
    { name: "Travel eSIM Starter Guide", slug: "travel-esim-starter-guide", brand: "Roamora", category: "Connectivity", priceRange: "Free", bestFor: "Understanding eSIMs before you buy" },
  ...
  const parisBlocks: ContentBlock[] = [
    ...
    { type: "ul", items: ["Metro & RER: single ticket €2.15; a carnet of 10 is cheaper", "Buses: scenic and slower, but excellent for the boulevards", "Walking: the real way to see Paris — bring comfortable shoes", "Bike share: the Vélib' network is great for flat routes along the Seine"] },
  ...
  if (itineraryRow) {
    const days: {
      ...
      { dayNumber: 4, description: "Morning on the hill, lunch in Pigalle, then a sunset cruise along the Seine before departure.", activities: ["Montmartre & Sacré-Cœur", "Seine evening cruise"], restaurants: ["Lunch in Pigalle"], hotel: "Hôtel Joke Astotel", transportation: "Metro + cruise", estimatedCost: 90 },
    ];
    for (const d of days) {
